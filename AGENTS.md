...
- Import of a `SkeletonDataAsset` does **not** bake. Use Inspector `Bake Skeleton Data` when the component has no container, `Tools/GPUSpineSkin/Bake All Skeleton Data` for the project, or `Rebake Selected` / `Force Rebake Selected` on a selection.
